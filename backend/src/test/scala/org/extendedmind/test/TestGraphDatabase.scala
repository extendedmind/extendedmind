...
trait TestGraphDatabase extends GraphDatabase {
  ...
  def insertTestData(testDataLocation: Option[String] = None) {
    withTx {
      implicit neo =>
        ...
        val testDataNode = getNode(initializeResult._2.get, OwnerLabel.COLLECTIVE).right.get
        val timoNode = getNode(initializeResult._3.get, OwnerLabel.USER).right.get
  ...
  def createCollective(creator: Node, title: String, description: Option[String], common: Boolean, displayName: Option[String], handle: Option[String], content: Option[String], format: Option[String], preferences: Option[OwnerPreferences])(implicit neo: DatabaseService): Node = {
