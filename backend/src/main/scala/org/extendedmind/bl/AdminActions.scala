...
trait AdminActions {
  ...
  def upgradePublishedNotes(userUUID: UUID, ownerUUID: UUID)(implicit log: LoggingAdapter): Response[CountResult] = {
    log.info("upgradePublishedNotes")
    val result = db.upgradePublishedNotes(userUUID, ownerUUID)
    if (result.isRight){
