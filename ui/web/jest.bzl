"Shows how you might create a macro for the autogenerated Jest rule"

load("@npm//jest:index.bzl", "jest", _jest_test = "jest_test")

def jest_test(name, srcs, deps, jest_config, jest_playwright_config, hub_bin, hub_port, **kwargs):
    "A macro around the autogenerated jest_test rule"
    templated_args = [
        "--no-cache",
        "--no-watchman",
        "--ci",
        "--colors",
    ]
    templated_args.extend(["--config", "$(rootpath %s)" % jest_config])
    for src in srcs:
        templated_args.extend(["--runTestsByPath", "$(rootpath %s)" % src])

    # HACK: Pass in extra args after "--" to use in jest.e2e.config.js
    templated_args.extend([
        "--",
        "--playwright-config", "$(rootpath %s)" % jest_playwright_config,
        "--hub-bin", "$(rootpath %s)" % hub_bin,
        "--hub-port", hub_port])

    data = [jest_config, jest_playwright_config, hub_bin] + srcs + deps + ["jest-reporter.js"]
    _jest_test(
        name = name,
        data = data,
        templated_args = templated_args,
        **kwargs
    )

    # This rule is used specifically to update snapshots via `bazel run`
    jest(
        name = "%s.update" % name,
        data = data,
        templated_args = templated_args + ["-u"],
        **kwargs
    )
